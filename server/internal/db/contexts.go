...
// ContextFilterAdapter wraps DB to implement mcpproxy.ContextFilter interface
type ContextFilterAdapter struct {
	db *DB
}

// NewContextFilterAdapter creates a new adapter for context filtering
func NewContextFilterAdapter(db *DB) *ContextFilterAdapter {
	return &ContextFilterAdapter{db: db}
}
...
func (a *ContextFilterAdapter) IsToolEnabledInContext(contextName, serverName, toolName string) (bool, error) {
	return a.db.IsToolEnabledInContext(contextName, serverName, toolName)
}
...
func (a *ContextFilterAdapter) GetEnabledServersForContext(contextName string) ([]string, error) {
	servers, err := a.db.GetEnabledServersForContext(contextName)
	if err != nil {
		return nil, err
	}
...
func (a *ContextFilterAdapter) GetDefaultContext() (string, error) {
	ctx, err := a.db.GetDefaultContext()
	if err != nil {
		return "", err
	}
	if ctx == nil {
